...
export const CONTROLLED_TOOL_CATALOG: ReadonlyMap<string, AgentToolDefinition> = new Map([
  [
    "find_employee",
    {
      type: "function",
      ...
      parameters: {
        type: "object",
        properties: {
          query: { type: "string", description: "Nombre o número de empleado" }
        },
        required: ["query"],
...
export function controlledTools(discoveredNames: string[]): AgentToolDefinition[] {
  ...
  if (tools.length !== CONTROLLED_TOOL_CATALOG.size) {
    ...
    throw new Error(`Required MCP tools are unavailable: ${missing.join(", ")}`);
  }
