...
import { evalRunSchema, type EvalCheck, type EvalResult, type EvalRun } from "./contracts.js";
...
function toolEvidence(answer: AgentAnswer): { count: number | null; records: unknown[] } {
  const toolEvent = answer.trace.find((item) => item.name === "mcp.tool.call.completed");
...
function evaluate(definition: CaseDefinition, answer: AgentAnswer, durationMs: number): EvalResult {
  const evidence = toolEvidence(answer);
  const checks: EvalCheck[] = [
    { name: "grounded", passed: answer.grounded === true, expected: true, actual: answer.grounded },
    { name: "approved_tool", passed: answer.toolsUsed.includes(definition.expectedTool), expected: definition.expectedTool, actual: answer.toolsUsed },
    { name: "database_count", passed: evidence.count === definition.expectedCount, expected: definition.expectedCount, actual: evidence.count }
  ];

  if (definition.expectedEmployeeNumber) {
    const found = evidence.records.some((record) =>
      typeof record === "object" && record !== null && "employeeNumber" in record && record.employeeNumber === definition.expectedEmployeeNumber
    );
    checks.push({ name: "fresh_record_observed", passed: found, expected: definition.expectedEmployeeNumber, actual: found });
  }
  if (definition.expectsExplicitEmpty) {
    const explicit = /no\s+(se\s+)?(encontr|hay)|sin\s+resultados|0\s+resultados/i.test(answer.answer);
    checks.push({ name: "explicit_empty_answer", passed: explicit, expected: "explicit no-result statement", actual: answer.answer });
  }
...
async function runCase(agent: AgentRunner, definition: CaseDefinition): Promise<EvalResult> {
  ...
  try {
    ...
    return {
      ...
      checks: [{ name: "execution_completed", passed: false, expected: true, actual: message }],
      evidence: {
...
export async function runAgentEvals(
  ...
  results.push(await runCase(agent, {
