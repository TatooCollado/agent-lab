...
export class PostgresHrRepository implements HrRepository {
  ...
  async findEmployees(query: string): Promise<LimitedResult<EmployeeDirectoryRecord>> {
    type Row = {
      employee_id: string;
  ...
  async listLateArrivals(
    ...
    const result = await this.pool.query<Row>(
      `SELECT
       ...
       LIMIT $4`,
      [period.startInclusive, period.endExclusive, employeeNumber ?? null, EVENT_LIMIT]
    );
